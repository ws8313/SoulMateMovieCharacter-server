...
character_movies_fields = MbtiCharacter.model('Character and Movies List', {
    'character_movies': fields.List(fields.List(fields.List(fields.String), example=
    {
        "character_name": "Dobby",
            "movies": [
                {
...
def ShowCharacter(mbti):
    ...
    random_characters_info = random.sample(characters_info, 4)
...
def MovieList(id, name):
    movie_list = db.session.query(CharacterInMovie.movie_id).filter(CharacterInMovie.character_id == id)
...
class MovieSatisfactionList(Resource):
    ...
    def post(self):
        ...
        isExistSatisfaction = db.session.query(Satisfaction).filter(Satisfaction.user_id == current_user, Satisfaction.movie_id == satisfaction_list[0]).first()

        if isExistSatisfaction:
                isExistSatisfaction.user_rating = satisfaction_list[1]
        else:
            db.session.add(Satisfaction(current_user, satisfaction_list[0], satisfaction_list[1]))
        db.session.commit()

        # for satisfaction in satisfaction_list:
        #     isExistSatisfaction = db.session.query(Satisfaction).filter(Satisfaction.user_id == current_user, Satisfaction.movie_id == satisfaction[0]).first()
        #     print(isExistSatisfaction)
        #     print(satisfaction)
        #     if isExistSatisfaction:
        #         isExistSatisfaction.user_rating = satisfaction[1]
        #     else:
        #         db.session.add(Satisfaction(current_user, satisfaction[0], satisfaction[1]))
        #         print(current_user, satisfaction[0], satisfaction[1])
        # db.session.commit()
        # print(db.session)
...
#         result = session.get('same_characters')
#         print(result)
...
# @MbtiCharacter.route('/movie_list/<int:id>/<string:name>')
# @MbtiCharacter.route('/movie_list?<int:id>&<string:name>')
@MbtiCharacter.route('/character_movie_list')
# @MbtiCharacter.doc(params={'id': '등장한 영화 리스트를 볼 캐릭터의 id', 'name': '등장한 영화 리스트를 볼 캐릭터의 이름' })
class MovieListWithCharacters(Resource):
    @jwt_required()
    # @login_required
    @MbtiCharacter.response(200, 'Success', character_movies_fields)
    @MbtiCharacter.response(500, 'fail')
    ...
    def get(self):
        ...
        character_movies_list = MovieList(character_id, character_name)

        return {
            'character_movies': character_movies_list
        }
